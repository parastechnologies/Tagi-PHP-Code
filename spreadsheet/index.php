...
//$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("uploads/RetailerAddproduc121.xls");
...
$data = array(1,$sheet->toArray(null,true,true,true));
$params = "(product_code,product_name,product_brand,description,category,is_fragile,retailer_id,status,productUniqueID)"; 
$params2="(product_id, productUniqueID, size, weight, length, height, width, quantity, price, is_dimension,color)";
    ...
    foreach($data[$i] as $dd){
        if($dd['B'] != 'No.' && $dd['B'] != ''){
            ...
            if(empty($product_id))
            {
                ...
                values ('$product_code','$product_name','$product_brand','$description','$catid','$fragile1','$retailer_id',1,'$newProductUniqueID')";
                $result=mysqli_query($con,$QUERY);
                $product_id=mysqli_insert_id($con);
            }
            
            $Sqlquery="insert into product_details $params2 values 
            ('$product_id','$newProductUniqueID','$size','$weight','$length','$height','$width','$quantity','$product_price','$is_dimension','$color') ";
...
if($results){
    $response=array('success' => 1,'msg' => 'Sheet Uploaded Successfully');
}
else 
{
    $response=array('success' => 0,'msg' => 'Product Not Created','data' => NULL);  
     ...
     $response=array('success' => 0,'msg' => 'upload sheet error','data' => NULL); 
}
	echo json_encode($response);
/*$params = "(product_code,product_name,product_brand,description,product_price,category,size,weight,length,width,height,is_fragile,retailer_id,status)"; 
$sheetCount = count($data[1]);
 for ($i = 0; $i <= $sheetCount; $i ++) {
     ...
     foreach($data[$i] as $dd){
         ...
         $category = mysqli_real_escape_string($con,$dd['C']);
         $weight = mysqli_real_escape_string($con,$dd['E']);
         $size = mysqli_real_escape_string($con,$dd['F']);
         $length = mysqli_real_escape_string($con,$dd['G']);
         $width = mysqli_real_escape_string($con,$dd['H']);
         $height = mysqli_real_escape_string($con,$dd['I']);
         $product_price = mysqli_real_escape_string($con,$dd['J']);
         $fragile = mysqli_real_escape_string($con,$dd['K']);
         $retailer_id = mysqli_real_escape_string($con,$retailer_id);
         $category_description = mysqli_real_escape_string($con,' ');
        // }
     }
 }
 echo  $QUERY = "insert into products $params 
        values ('$product_code','$product_name','$product_brand','$description','$product_price','$catid','$size','$weight','$length','$width','$height','$fragile','$retailer_id',1)";
        */
        /* $sql = mysqli_query($con,"select id,name from product_categories where name = '$category'");
